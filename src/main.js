// ==UserScript==
// @name         Claude Usage Limit Presence
// @namespace    claude-usage-limit-presence
// @version      0.1.0
...
function updateMinimizeButtonTitle() {
    ...
    const fiveHourSummary = (fiveHourRow?.textContent ?? '--') +
        (fiveHourRow?.title ? '  (resets: ' + fiveHourRow.title + ')' : '');
    const sevenDaySummary = (sevenDayRow?.textContent ?? '--') +
        (sevenDayRow?.title ? '  (resets: ' + sevenDayRow.title + ')' : '');
...
async function fetchStats() {
    ...
    try {
        ...
        if (fiveHourRow) fiveHourRow.title = formatResetTime(usageData?.five_hour?.resets_at);
        if (sevenDayRow) sevenDayRow.title = formatResetTime(usageData?.seven_day?.resets_at);
        ...
        console.warn('[claude-usage-panel] fetch failed:', fetchError);
 ...
 */
function init() {
    console.log('Claude Usage Limit Presence');
    createWidget();
    hideWarning();
